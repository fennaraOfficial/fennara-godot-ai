...
- For read-only `run_scene_edit_script` inspection, use `mode: "inspect"` and `ctx.log(...)`. Do not call direct mutation APIs. The context rejects `ctx.mark_modified()` and mutating helpers.
