...
pub(crate) const PROVIDER_ID: &str = ProviderId::ANTHROPIC;
...
pub(crate) fn provider_definition(api_key: Option<&str>) -> ProviderDefinition {
    ProviderDefinition {
        ...
        auth: api_key
            ...
            .map(|key| Auth::InlineHeader {
                name: API_KEY_HEADER.to_string(),
                value: key.trim().to_string(),
            })
            .unwrap_or_else(|| Auth::EnvHeader {
                name: API_KEY_HEADER.to_string(),
...
pub(crate) fn model_definition(model_id: &str, display_name: Option<String>) -> ModelDefinition {
    ...
    ModelDefinition {
        id: ModelId::new(model_id).expect("Anthropic model id is valid"),
