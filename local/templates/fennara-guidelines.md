...
`runtime_session` starts, checks, or stops one daemon-managed windowed Godot runtime session. Runtime sessions enable Godot's local debugger with error breaks ignored so debugger-aware addons can send debugger messages without requiring an attached editor session; normal errors and warnings are still captured in `runtime_session.log`. Use the `runtime_session` tool schema for exact actions, arguments, startup wait behavior, log excerpt behavior, and result fields.
