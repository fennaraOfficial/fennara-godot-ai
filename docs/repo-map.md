...
| `local/crates/fennara-daemon/src/runtime_daemon/chat/checkpoints/` | Private Git tree capture and durable turn-checkpoint lifecycle for built-in chat recovery, including project isolation, changed-path tracking, generated-cache exclusions, coverage, capture limits, and retention. |
