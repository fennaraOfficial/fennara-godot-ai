...
| `local/crates/fennara-daemon/src/runtime_daemon/chat/checkpoints/` | Private Git tree capture and durable turn recovery for built-in chat, including project isolation, changed-path tracking, generated-cache exclusions, coverage, limits, retention, conflict detection, path-scoped restore, crash reconciliation, and redo. |
| `local/crates/fennara-daemon/src/runtime_daemon/chat/turn_recovery.rs` / `store/turn_recovery.rs` | Internal Undo/Redo request orchestration and durable recovery journal, capability, rewind-boundary, and replacement-tail transactions. |
