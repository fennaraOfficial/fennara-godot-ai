<!-- fennara-i18n: locale=ko source=docs/release.md sha256=23e966649d82728c9ab9802dd2957bb64c1b13fd3e7ae44bd513835bc063ff77 -->
...
현재 릴리스 정책은 안정 매니페스트에 CLI `0.3.12`, 스테이징 매니페스트에
CLI `0.3.8`을 요구합니다. 안정 검색은 폐기된 `latest` 태그를 더 이상
해석하지 않습니다. 안정 `0.3.12`에는 수정된 업데이트 검증, 버전 전환 사전
점검, Windows 작업 저널 처리가 필요합니다. `0.3.12-pr.123.1` 같은
스테이징 후보는 SemVer에서 안정 `0.3.12`보다 낮게 비교되므로, 최초 실행
